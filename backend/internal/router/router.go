...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	batchClaimRepository := repository.NewBatchClaimRepository(db)
	...
	faultEventService := service.NewFaultEventService(faultEventRepository, batchClaimRepository, securityService)
