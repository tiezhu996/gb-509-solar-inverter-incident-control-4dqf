...
func handleError(c *gin.Context, err error) {
	...
	case errors.Is(err, service.ErrBatchRejected):
		if rejection, ok := service.AsBatchClaimRejection(err); ok {
			util.FailWithMeta(c, http.StatusUnprocessableEntity, "batch_rejected", rejection.Detail, rejection)
			return
		}
		util.Fail(c, http.StatusUnprocessableEntity, "batch_rejected", err.Error())
