...
- 故障事件列表支持批量认领：操作员一次勾选最多 20 条待认领故障并提交统一原因；后端在单事务内完成全部 open → acknowledged，同一场站仍为 tripped 的逆变器联动转为 warning，每一项变化逐条写审计。编号不存在、状态已变化、版本过期或编号重复时整批拒绝（422）并返回逐条阻断明细，不产生部分更新。
